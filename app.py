...
def send_message(data):
    ...
    # Обновляем непрочитанные и отправляем уведомление
...
:root {
    ...
    --primary-gradient: linear-gradient(135deg, #0088cc, #00b4d8);
    --text: #ffffff;
...
.msg.self .msg-bubble .edited { color: rgba(255,255,255,0.5); }
.msg-time {
    font-size: 9px;
    color: var(--text-secondary);
    text-align: right;
...
.msg.self .msg-time { color: rgba(255,255,255,0.5); }
...
.fab svg { width: 28px; height: 28px; fill: none; stroke: #fff; stroke-width: 2.5; stroke-linecap: round; stroke-linejoin: round; }
...
@media (max-width: 480px) {
    ...
    .push-notification { min-width: unset; width: 92%; }
}
</style>
</head>
<body>
...
socket.on('push_notification', (data) => {
    ...
    // Переключаем на чаты если пользователь в контактах
...
socket.on('new_message', (data) => {
    ...
    renderChats();
    // Если мы в контактах - переключаем на чаты
    if (document.getElementById('pageUsers').classList.contains('active')) {
        // Не переключаем, просто обновляем бейдж
    }
});

// ... остальные socket события (такие же как раньше, но с обновленными названиями)
// Я сократил для длины, но все они должны быть из предыдущей версии
...
function handleAvatar(e) {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (ev) => { currentAvatar = ev.target.result; socket.emit('update_avatar', { name: currentUser, avatar: ev.target.result }); renderSettings(); showToast('Аватар обновлен!'); };
    reader.readAsDataURL(file);
    e.target.value = '';
}
